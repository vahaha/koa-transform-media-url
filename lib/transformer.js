...
exports.init = async getDomainList => {
    domains = await Promise.resolve(getDomainList())

    if (!domains.length) {
        return
    }

    baseUrlRegex = domains.map(domain => {
        const fixDomain = domain.replace(/\./g, '\\.').replace(/\//g, '\\/')
        const reg = new RegExp(`${fixDomain}\\/`)

        return reg
    })

    urlRegex = domains.map(domain => {
        const fixDomain = domain.replace(/\./g, '\\.').replace(/\//g, '\\/')
        const formular = `${fixDomain}\\/${strRegexUuid}.+`
        const regexEndpoint = new RegExp(formular, 'g')

        return regexEndpoint
    })

    arrSourceRegex = domains.map(domain => {
        const fixDomain = domain.replace(/\./g, '\\.').replace(/\//g, '\\/')
        const testEndpoint = `src\\s*=\\s*"${fixDomain}\\/${strRegexUuid}\\/.+?"`
...
exports.transformUrlToIdInContent = async function transformUrlToIdInContent(
    ...
    const missedSignedUrls = await getAndCacheMapping(
        tid,
        missedPaths,
        fnGetIdByUrls,
    )
...
async function getAndCacheMapping(tid, items, fnTransform) {
    ...
    const setSignedUrls = await Promise.map(
        batchPaths,
        batchPath => fnTransform(tid, batchPath),
        {
            concurrency: 1,
        },
    )
